...
export default function RocketCore({ initialLaunchTime }) {
	...
	const [timeDiff, setTimeDiff] = useState(currentDate - initialLaunchTime)

	useEffect(() => {
		setInterval(() => {
			setCurrentDate((currentDate) => initialLaunchTime + currentDate)
			setTimeDiff(currentDate - initialLaunchTime)
		}, 500)
	}, [initialLaunchTime, currentDate])

	const timeToPositionPercent = () => {
		if (timeDiff >= MS_TO_TAKEOFF) {
			return FINAL_POSITION_BOTTOM_VAL
		}

		return `calc(200px + ${((timeDiff / MS_TO_TAKEOFF) * 100).toFixed(0)}%)`
	}
